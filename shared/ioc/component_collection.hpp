...
namespace ioc {
	...
	public:
		...
		//        사용 방법에 따라 논의 필요.
        Result<void> add(const std::shared_ptr<IComponent>& component)
        {
        ...
        Result<void> add(std::shared_ptr<IComponent>&& component)
        {
            std::lock_guard<std::mutex> lock(mutex_);
            const auto interface_type = component->interfaceType();
		...
		template<typename I>
		std::shared_ptr<I> getService(intptr_t scoped_key = 0) const
		{
			auto name = std::string(getTypeId<I>());
		...
		template<typename I>
		std::shared_ptr<I> getService(const std::string& name, intptr_t scoped_key = 0) const
		{	
            std::lock_guard<std::mutex> lock(mutex_);
			auto interface_type = getTypeId<I>();
			auto it = collection_.find(interface_type);
			...
			return base_component ? base_component->createService(scoped_key) : nullptr;
		}

		// key로 관리되는 전체 component의 instance들을 만료시킨다.(폐기한다).
		// 실제 동작은 Component의 Factory에서 관리하는 instance를 폐기한다.
		Result<void> expiredInstance(intptr_t key = 0);
