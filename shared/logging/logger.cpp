...
Result<void> Logger::init(logging::Type logger_type, const std::string& filename) {
    try {
    ...
    switch (logger_type)
    {
    case logging::Type::Console:
        logger_ = std::make_shared<ConsoleBackend>();
        break;
#ifdef USE_SPDLOG
...
Result<void> Logger::apply() {
    ...
    for (auto it : config_["log"]) {
        ...
        // 레벨 적용
        if (node["level"]) {
            logger_->setLevel(tag, toLevel(node["level"].as<std::string>()));
        }
    }
}

Result<void> Logger::configureSink(const std::string& tag, const YAML::Node& sink) {
    if (!logger_) return;
    ...
    if (type == "console") {
        ...
        logger_->setConsoleSink(tag);
    } else if (type == "file") {
        logger_->setFileSink(tag, sink["filename"].as<std::string>());
    } else if (type == "rotating_file") {
        logger_->setRotatingFileSink(tag,
            sink["filename"].as<std::string>(),
            sink["max_size"].as<size_t>(),
            sink["max_files"].as<size_t>());
    } else if (type == "syslog") {
        logger_->setSyslogSink(tag,
            sink["ident"] ? sink["ident"].as<std::string>() : tag);
    } else if (type == "udp") {
        logger_->setUdpSink(tag,
            sink["host"].as<std::string>(),
            sink["port"].as<int>());
    } else if (type == "udp_json") {
        logger_->setUdpJsonSink(tag,
            sink["host"].as<std::string>(),
            sink["port"].as<int>());
    } else if (type == "loki") {
        logger_->setLokiSink(tag,
